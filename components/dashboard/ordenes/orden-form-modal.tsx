...
export default function OrdenFormModal({ isOpen, onClose, onSuccess, orden }: OrdenFormModalProps) {
  ...
  const calcularTotales = (detalles: DetalleOrden[]) => {
    const subtotalCalculado = detalles.reduce((sum, item) => sum + item.subtotal, 0)
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell className="text-right">
                        <Input
                          ...
                          onChange={(e) => updateCantidad(index, Number.parseInt(e.target.value))}
