...
export default function OrdenesListPage() {
  ...
  const [editedNotes, setEditedNotes] = useState<Map<string | number, string>>(
    new Map()
  );
  ...
  const handleCambiarEstado = async (
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(
          result.error || "Error desconocido al cambiar estado de la orden."
        );
      }
    } catch (error) {
  ...
  const handleGuardarNotas = async () => {
    ...
    try {
      const detallesParaActualizar = Array.from(editedNotes.entries()).map(
        ([id, notas]) => ({ id, notas })
      );
  ...
  const filteredOrdenes = ordenesValidas.filter(
    (o) => activeTab === "todas" || o.estado === o.estado
  );
  const ordenesPendientes = ordenesValidas.filter(
    (o) => o.estado === "pendiente"
  );
  const ordenesEnProceso = ordenesValidas.filter(
    (o) => o.estado === "preparando"
  );
  const ordenesCompletadas = ordenesValidas.filter(
  ...
  const getTimeElapsed = (fechaOrden: string) => {
    ...
    const diffInMinutes = Math.floor(
      (new Date().getTime() - new Date(fechaOrden).getTime()) / 60000
    );
  ...
  const OrdenCard = ({ orden }: { orden: Orden }) => (
    <Card
      className={`mb-4 transition-all duration-200 hover:shadow-lg ${getStatusCardClass(
        orden.estado
      )}`}
    >
      ...
      <CardContent className="pt-0">
        <div className="space-y-2 mb-4">
          {Array.isArray(orden.detalles_orden) &&
            orden.detalles_orden.slice(0, 5).map((detalle, index) => (
              <div key={index} className="bg-white p-2 rounded border">
                ...
                {detalle.notas && (
                  <p className="text-xs text-orange-700 italic flex items-center gap-1 mt-1 pl-6">
          ...
          {Array.isArray(orden.detalles_orden) &&
            orden.detalles_orden.length > 5 && (
              <div className="text-xs text-gray-500 text-center">
                +{orden.detalles_orden.length - 5} productos más
              </div>
            )}
        </div>
        {orden.notas && (
        ...
        <div className="flex items-center gap-2 mb-4 text-sm text-gray-600">
          ...
          <span>
            {orden.metodo_pago === "efectivo"
              ...
              ? "Tarjeta de Débito"
              : orden.metodo_pago === "puntos_recompensa" // <-- AÑADIR ESTA LÍNEA
              ? "Puntos de Recompensa" // <-- AÑADIR ESTA LÍNEA
              : orden.metodo_pago}
          </span>
        </div>
        ...
        <TabsContent value="lista" className="space-y-4">
          ...
          >
            <TabsList className="mb-4 flex flex-wrap">
              ...
              <TabsTrigger value="preparando">En Proceso</TabsTrigger>
                        ...
                        <TableBody>
                          {filteredOrdenes.map((orden) => (
                            <TableRow key={orden.id}>
                              ...
                              <TableCell>
                                <Badge
                                  className={`${getBadgeVariant(orden.estado)}`}
                                >
                                  {orden.estado === "pendiente"
                                    ? "Pendiente"
                              ...
                              <TableCell className="text-right">
                                <div className="flex justify-end gap-2">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      handleVerYEditarDetalles(orden)
                                    }
                                    className="h-8 px-2"
                                  >
                                    <Eye className="h-4 w-4 mr-1" />
                                  ...
                                  {orden.estado === "pendiente" && (
                                    <>
                                      <Button
                                        ...
                                        } // CAMBIADO: De "en_proceso" a "preparando"
                                        disabled={isChangingStatus}
        ...
        <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-amber-900">
              Detalles y Notas de la Orden #{selectedOrden?.id}
            </DialogTitle>
          </DialogHeader>
          {isDetallesLoading ? (
            <div className="text-center py-8">
                ...
                <>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedOrden.clientes
                          ? `${selectedOrden.clientes.nombre} ${
                              selectedOrden.clientes.apellido || ""
                            }`
                          : selectedOrden.id_cliente
                          ? `Cliente ID: ${selectedOrden.id_cliente}`
                          : "Cliente no disponible"}
                      </p>
                      {selectedOrden.clientes?.email && (
                        <p className="text-sm">
                          {selectedOrden.clientes.email}
                        </p>
                      )}
                      {selectedOrden.clientes?.telefono && (
                        <p className="text-sm">
                          {selectedOrden.clientes.telefono}
                        </p>
                      )}
                    ...
                    <div>
                      ...
                      <p>
                        ...
                        {selectedOrden.metodo_pago === "efectivo"
                          ...
                          ? "Puntos"
                          : selectedOrden.metodo_pago === "puntos_recompensa" // <-- AÑADIR ESTO
                          ? "Puntos de Recompensa" // <-- Y ESTO
                  ...
                  {selectedOrden.notas && (
                    <div className="border-t pt-4 mt-4">
                      ...
                      <p className="text-sm mt-1">{selectedOrden.notas}</p>
                    </div>
                  )}
                  <div>
                    <h3 className="font-medium text-gray-500 mb-2">
                      Productos y Notas Editables
                    </h3>
                      ...
                      <ScrollArea className="max-h-[300px] pr-4">
                        <div className="space-y-3">
                          {detallesOrden.map((d) => (
                            <div
                              key={d.id}
                              className="p-3 border rounded-md bg-amber-50/50"
                            >
                              ...
                              <div className="mt-2">
                                ...
                                <Input
                                  ...
                                  onChange={(e) =>
                                    handleNoteChange(d.id, e.target.value)
                                  }
                  ...
                  <div className="flex flex-wrap gap-2 justify-end">
                    {selectedOrden.estado === "pendiente" && (
                      <>
                        <Button
                          variant="outline"
                          onClick={() =>
                            handleCambiarEstado(selectedOrden.id, "preparando")
                          } // CAMBIADO: De "en_proceso" a "preparando"
                          disabled={isChangingStatus}
                        ...
                        <Button
                          variant="outline"
                          onClick={() =>
                            handleCambiarEstado(selectedOrden.id, "cancelada")
                          }
                          disabled={isChangingStatus}
                          className="border-red-200 text-red-800 hover:bg-red-50"
                        >
                    ...
                    {selectedOrden.estado === "preparando" && ( // CAMBIADO: La condición de "en_proceso" a "preparando"
                      <Button
                        variant="outline"
                        onClick={() =>
                          handleCambiarEstado(selectedOrden.id, "completada")
                        }
                        disabled={isChangingStatus}
