...
export default function VentasPage({ onVentaCompleted }: VentasPageProps) {
  ...
  const [orderToInvoice, setOrderToInvoice] = useState<OrdenTemporal | null>(
    null
  );
  const [clientForInvoice, setClientForInvoice] = useState<Cliente | null>(
    null
  );
  ...
  const [selectedImpuestoRate, setSelectedImpuestoRate] =
      ...
      if (isMounted.current && Array.isArray(data)) {
        ...
        if (categoriesFromProducts.length > 0) {
          setLocalCategories((prev) => {
            const newCategories = [
              ...new Set([...prev, ...categoriesFromProducts]),
            ].sort();
            try {
      ...
      if (isMounted.current && Array.isArray(data) && data.length > 0) {
        ...
        if (categoryNames.length > 0) {
          setLocalCategories((prevCats) => {
            const combined = [
              ...new Set([...prevCats, ...categoryNames]),
            ].sort();
            try {
              localStorage.setItem(
                "ventasCategories",
  ...
  const categories = useMemo(() => {
    ...
    if (Array.isArray(productos) && productos.length > 0)
      return extractCategoriesFromProducts(productos);
    return [];
  ...
  const filteredProductos = useMemo(() => {
    ...
    return productos.filter((producto) => {
      ...
      const matchesCategory =
        activeTab === "all" || producto.categoria === activeTab;
  ...
  const updateCartItemNotes = useCallback(
    (id: number | string, notas: string) => {
      setCart((prevCart) =>
        prevCart.map((item) => (item.id === id ? { ...item, notas } : item))
      );
    },
    []
  );
  ...
  const handleCheckout = useCallback(() => {
    if (cart.length === 0) {
      toast.error(
        "Carrito vacío. Agrega productos al carrito antes de continuar."
      );
      return;
    }
            ...
            <CardContent className="flex-grow overflow-hidden p-0 sm:p-6">
              ...
              >
                ...
                <ScrollArea className="h-[calc(100vh-350px)] sm:h-[500px] flex-grow px-4 sm:px-0">
                  <TabsContent
                    value={activeTab}
                    className="m-0 data-[state=active]:block h-full"
                  >
                            ...
                            <div className="aspect-square relative bg-amber-50 w-full h-48 overflow-hidden">
                              {producto.imagen_url ? (
                                <>
                                  <img
                                    src={
                                      producto.imagen_url || "/placeholder.svg"
                                    }
                                    alt={producto.nombre}
                ...
                <ScrollArea className="h-[calc(100vh-450px)] sm:h-[400px]">
                  <div className="space-y-4 pr-2">
                    {cart.map((item) => (
                      <div
                        key={`cart-${item.id}`}
                        className="border rounded-md p-3 bg-amber-50/50"
                      >
            ...
            <CardFooter className="flex-shrink-0 border-t pt-4 flex flex-col">
              <div className="w-full space-y-2">
                <div className="space-y-2">
                  <Label htmlFor="carrito-impuesto-rate">
                    Tasa de Impuesto
                  </Label>
                  <Select
                    value={selectedImpuestoRate.toString()}
                    onValueChange={handleImpuestoRateChange}
                  >
                    <SelectTrigger
                      id="carrito-impuesto-rate"
                ...
                <div className="flex justify-between text-sm">
                  <span>
                    Impuesto ({(selectedImpuestoRate * 100).toFixed(0)}%):
                  </span>
